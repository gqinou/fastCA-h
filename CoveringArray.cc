...
void CoveringArray::actsInitialize(const std::string file_name) {
  ...
  coverByLineindex.resize(coverage.tupleSize());
  ...
  tmpPrint();
  oneCoveredTuples.initialize(specificationFile, array.size()); //construct coverying array
  oneCoveredTuples.pushOneCoveredTuple(coverage, coverByLineindex,index);
/*#ifndef NDEBUG
  int i = 0;
  for (auto &line : array) {
    std::cout << i++ << ": ";
...
void CoveringArray::replaceRow(const unsigned lineIndex,
  ...
  for (std::vector<unsigned> columns = combinadic.begin(strength);
    ...
    uncover(coverage.encode(columns, tmpTuple), lineIndex, index);
  }
  ...
  for (std::vector<unsigned> columns = combinadic.begin(strength);
    ...
    cover(coverage.encode(columns, tmpTuple), lineIndex,index);
  }
  ...
  for (size_t lineIndex = 0; lineIndex < array.size();) {
     if (oneCoveredTuples.oneCoveredCount(lineIndex) == 0 &&
      ...
      for (std::vector<unsigned> columns = combinadic.begin(strength);
        ...
        uncover(encode, lineIndex, index);
          }
      ...
      oneCoveredTuples.exchange_row(lineIndex, array.size() - 1);
         if(lineIndex<array.size() - 1){
             for (std::vector<unsigned> columns = combinadic.begin(strength);
                 ...
                 {
                     if(coverByLineindex[encode][t]==array.size() - 1)
                     {
                         coverByLineindex[encode][t]=lineIndex;
                     }
...
void CoveringArray::removeOneRowRandom() {
    ...
    if(rowToremoveIndex<array.size() - 1){
        for (std::vector<unsigned> columns = combinadic.begin(strength);
             columns[strength - 1] < options.size(); combinadic.next(columns)) {
            for (unsigned j = 0; j < strength; ++j) {
                tmpTuple[j] = array[rowToremoveIndex][columns[j]];
            ...
            for(int t=0;t<coverByLineindex[encode].size();t++)
            {
                if(coverByLineindex[encode][t]==array.size() - 1)
                {
                    coverByLineindex[encode][t]=rowToremoveIndex;
                }
...
CoveringArray::multiVarRow(const std::vector<unsigned> &sortedMultiVars,
  ...
  if (sortedMultiVars.size() >= strength) {
    for (std::vector<unsigned> changedColums = combinadic.begin(strength);
      ...
      if (change) {
        cover(tmpToCoverEncode, lineIndex,index);
        uncover(tmpToUncoverEncode, lineIndex,index);
      } else {
...
void CoveringArray::replace(const unsigned var, const unsigned lineIndex) {
  ...
  for (std::vector<unsigned> columns = combinadic.begin(strength - 1);
    ...
    cover(tmpTupleToCoverEncode, lineIndex,index);
    uncover(tmpTupleToUncoverEncode, lineIndex,index);
  }
  line[varOption] = var;
...
void CoveringArray::cover(const unsigned encode, const unsigned oldLineIndex,const unsigned index) {
  coverage.cover(encode);
  coverByLineindex[encode].push_back(oldLineIndex);
  unsigned coverCount = coverage.coverCount(encode);
  if (coverCount == index) {
      uncoveredTuples.pop(encode);
      oneCoveredTuples.push(encode, oldLineIndex, coverage.getTuple(encode),coverByLineindex);
  }
    if (coverCount == index + 1) {
    coverByLineindex[encode].pop_back();
    ...
    for (size_t lineIndex = 0; lineIndex < array.size(); ++lineIndex) {
      ...
      if (match) {
        oneCoveredTuples.pop(encode, lineIndex,tuple,coverByLineindex,array.size());
        coverByLineindex[encode].push_back(oldLineIndex);
        break;
      }
    }
...
void CoveringArray::uncover(const unsigned encode,
  ...
  for(int t=0;t<coverByLineindex[encode].size();t++)
    {
        if(coverByLineindex[encode][t]==oldLineIndex)
        { coverByLineindex[encode][t]=coverByLineindex[encode][coverByLineindex[encode].size()-1];
            coverByLineindex[encode].pop_back();
            break;
        }
    }
  ...
  if (coverCount == index - 1) {
    ...
    oneCoveredTuples.pop(encode, oldLineIndex, coverage.getTuple(encode),coverByLineindex,array.size());
      coverByLineindex[encode].pop_back();
  }   
  if (coverCount == index) {
    const std::vector<unsigned> &tuple = coverage.getTuple(encode);
    ...
    for (size_t lineIndex = 0; lineIndex < array.size(); ++lineIndex) {
      ...
      if (match) {
        oneCoveredTuples.push(encode, lineIndex,tuple,coverByLineindex);
        break;
      }
    }
  }
